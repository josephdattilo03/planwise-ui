...
export async function fetchRootFolder(): Promise<FolderNode | null> {
    ...
    if (folders.length === 0) {
        ...
        const boards = [
            { parentId: "folder-1", board: { id: "board-1", name: "Website Redesign", color: "#3b82f6" } },
            { parentId: "folder-1", board: { id: "board-2", name: "Mobile App", color: "#10b981" } },
            { parentId: "root", board: { id: "board-3", name: "Quick Notes", color: "#8b5cf6" } },
        ];
...
export async function fetchChildrenByParentId(parentId: string): Promise<WorkspaceNode[]> {
...
export function deleteFolder(folderId: string) {
    ...
    const updatedFolders = folders.filter((f: any) => !idsToRemove.includes(f.id));
    const updatedBoardNodes = boardNodes.filter((b: any) => !idsToRemove.includes(b.parentId));
...
// Helper function to build the full tree structure
export async function fetchWorkspaceTree(): Promise<FolderNode | null> {
    const allFolders = await fetchAllFolders();
    const allBoardNodes = await fetchAllBoardNodes();
    ...
    // First, add folders to their parents
    ...
    // Then, add boards to their parents
    allBoardNodes.forEach(boardNode => {
        const parent = folderMap.get(boardNode.parentId);
        if (parent) {
            parent.children.push(boardNode);
        }
    });
